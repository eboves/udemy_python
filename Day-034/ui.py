...
class QuizUi:
    def __init__(self):
        ...
        self.canvas_text = self.canvas.create_text(150,125, text="HELLO!! asdjkfhblaskbdc skjhdf ashdfglashdkflak asiudhfkalsjdbfla asjihdfaksjbd", font=("Ariel", 20, "italic"), fill="black")
        self.canvas.grid(row=1,column=0, columnspan=2, pady=50)

        right_btn_img = tk.PhotoImage(file=os.path.join(SCRIPT_DIR, "images/true.png"))
        self.right_btn = tk.Button(image=self.right_btn_img, highlightthickness=0, pady=20)
        self.right_btn.grid(row=2, column=0)

        wrong_btn_img = tk.PhotoImage(file=os.path.join(SCRIPT_DIR, "images/false.png"))
